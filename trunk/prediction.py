...
def predict_from_model(validation_generator):
    ...
    path_models_datetime = os.path.join(paths.MODELS, model_folder)  # resources/models/20181022.0055/
    path_prediction_datetime = os.path.join(paths.PREDICTION, model_folder) # resources/prediction/20181022.0055/
...
def predict_images(model, validation_generator, save_path):
    ...
    for i in range(n_sequences):
     ...
     #   datetime_name = paths.get_datetime_from_name(sequence_list[i])  # TODO int this case datetime is found in path, not only in name
        ...
        full_path_img = datetime_path + datetime_name + '.png'
        full_path_mat = datetime_path + datetime_name + '.mat'
        predicted_image = predicted_images[i, ::, ::, 0]

        # save image into mat file for matlab
        sio.savemat(full_path_mat, mdict={'arr': predicted_image})

        final_image = postprocess_image(predicted_image)
        final_image.save(full_path_img)
